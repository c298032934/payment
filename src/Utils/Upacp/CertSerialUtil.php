...
class CertSerialUtil
{
    ...
    public static function getSerial($fileData, &$errMsg)
    {
        ...
        if ($byte == 0xa0) { //version tag.
            $length = self::readLength($bindata);
            self::readData($bindata, $length);
            $byte = array_shift($bindata);
        }
        ...
        $length = self::readLength($bindata);
        $serial = self::readData($bindata, $length);
        return self::bytesToBigInteger($serial);
    }
    ...
    private static function readLength(&$bindata)
    {
        ...
        if ($byte < 0x80) {
            ...
            $length = self::bytesToInteger($lenBytes);
        }
        return $length;
    }
